...
public class RegisterController {
	...
	public ModelAndView showFormEdit(@ModelAttribute("findEmp") @Valid FindEmployee emp, BindingResult result) {
		...
		List<String> vm = service.getVM();
		modelAndView.addObject("registerBean", bean);
		modelAndView.addObject("getVm", vm);
		modelAndView.addObject("isEditPage", "Yes");
		return modelAndView;
	}
	...
	public ModelAndView saveEmployee(@ModelAttribute("registerBean") @Valid RegisterBean bean, BindingResult result) {
		...
		service.save(bean, "save");
		System.err.println(bean.toString());
		return new ModelAndView("EmployeeAddSuccess");
	}

	@RequestMapping(value = "/saveEmployee", method = RequestMethod.POST, params = "action=editEmployee")
	public ModelAndView editAndSaveEmployee(
			@ModelAttribute("registerBean") @Validated(value = Default.class) RegisterBean bean, BindingResult result) {
		if (result.hasErrors()) {
			ModelAndView modelAndView = new ModelAndView("newEmployee");
			modelAndView.addObject("isEditPage", "Yes");
			return modelAndView;
		}
		service.save(bean, "edit");
		System.err.println(bean.toString());
		return new ModelAndView("EmployeeAddSuccess");
	}
